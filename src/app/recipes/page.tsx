...
import { Plus, Search, SlidersHorizontal, Calendar, ArrowRight, X, Wand2 } from 'lucide-react';
    ...
    <div>
      ...
      <div className="flex flex-col gap-3 mb-5">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl md:text-3xl font-semibold tracking-tight text-foreground font-[family-name:var(--font-outfit)]">Recipes</h1>
          <div className="flex items-center gap-1.5">
            <button
              type="button"
              onClick={() => { setSearchOpen(prev => !prev); if (!searchOpen) setSearchQuery(''); }}
              className={`p-2 rounded-xl transition-colors ${searchOpen ? 'bg-primary/10 text-primary' : 'text-muted hover:text-foreground hover:bg-surface'}`}
            ...
            <div className="relative" ref={filterRef}>
              <button
                ...
                className={`p-2 rounded-xl transition-colors flex items-center gap-1 ${activeFilterCount > 0 ? 'bg-primary/10 text-primary' : filterOpen ? 'bg-surface text-foreground' : 'text-muted hover:text-foreground hover:bg-surface'}`}
                aria-label="Filter recipes"
              >
                <SlidersHorizontal className="w-5 h-5" strokeWidth={2} />
                {activeFilterCount > 0 && (
                  <span className="min-w-[16px] h-[16px] rounded-full bg-primary text-white text-[10px] font-bold flex items-center justify-center">{activeFilterCount}</span>
                )}
              </button>
              {filterOpen && (
                <div className="absolute right-0 top-full mt-2 w-72 rounded-2xl border border-border/50 bg-white shadow-xl z-50 p-4 flex flex-col gap-4">
                  <div>
                    <p className="text-sm font-medium text-foreground mb-2">Tags</p>
                    <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto">
                      {allTags.length === 0 ? (
                        <p className="text-xs text-muted">No tags in recipes yet.</p>
                      ) : (
                        allTags.map(tag => (
                          <button
                            key={tag}
                            ...
                            className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${filters.tags.includes(tag) ? 'bg-primary text-white' : 'bg-surface text-foreground hover:bg-surface-dark'}`}
                          >
                            {tag}
                          </button>
                        ))
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 rounded-xl border border-border/50 bg-white text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                    >
      ...
      {recipes.length >= 1 && !mealPlanHintDismissed && (
        <div className="mb-4 flex items-center gap-3 p-3 rounded-xl border border-primary/15 bg-primary/[0.03] animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="p-1.5 rounded-lg bg-primary/10 text-primary shrink-0">
            <Calendar className="w-4 h-4" />
          </div>
          <p className="flex-1 text-xs text-muted min-w-0">
            <span className="font-medium text-foreground">Ready for a full meal plan?</span>{' '}
            Turn your recipes into a weekly plan with shopping list.
          </p>
          <Link
            href="/meal-plan"
            className="shrink-0 inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-primary text-white text-xs font-medium hover:bg-primary-dark transition-colors"
          >
            Try it
      ...
      <button
        ...
        className="fixed bottom-28 md:bottom-8 right-4 md:right-8 bg-gradient-to-br from-primary to-primary-dark hover:from-primary-dark hover:to-[#1F4D28] text-white rounded-2xl shadow-xl shadow-primary/20 hover:shadow-2xl hover:shadow-primary/30 transition-all duration-200 hover:scale-105 active:scale-95 z-[90] flex items-center gap-2 px-5 py-3.5"
        aria-label="Create Recipe"
      >
        <Wand2 className="w-5 h-5" />
        <span className="font-semibold text-sm hidden sm:inline">New Recipe</span>
        <Plus className="w-5 h-5 sm:hidden" strokeWidth={2.5} />
      </button>
